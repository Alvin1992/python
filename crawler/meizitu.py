...
def get_soup(url):
    #获取网站的soup对象
    myheards = ['Mozilla/5.0 (Windows NT 5.2) AppleWebKit/534.30 (KHTML, like Gecko) Chrome/12.0.742.122 Safari/534.30',
    'Mozilla/5.0 (Windows NT 5.1; rv:5.0) Gecko/20100101 Firefox/5.0',
    'Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.2; Trident/4.0; .NET CLR 1.1.4322; .NET CLR 2.0.50727; .NET4.0E; .NET CLR 3.0.4506.2152; .NET CLR 3.5.30729; .NET4.0C)',
    'Opera/9.80 (Windows NT 5.1; U; zh-cn) Presto/2.9.168 Version/11.50',
    'Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN) AppleWebKit/533.21.1 (KHTML, like Gecko) Version/5.0.5 Safari/533.21.1',
    'Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.1; Trident/4.0; .NET CLR 2.0.50727; .NET CLR 3.0.04506.648; .NET CLR 3.5.21022; .NET4.0E; .NET CLR 3.0.4506.2152; .NET CLR 3.5.30729; .NET4.0C)']
    hearder = {"User-Agent": random.choice(myheards)}
    req = urllib2.Request(url, headers=hearder)
...
def main(page):
    #下载第page页的妹子图片
    print u'正在下载第%d页》》》》》》》'%page
    page_url = url+'/page/'+str(page)
    menu = get_menu(page_url)
    print u'>>>>>>>>>>>>第%d页 共有%d个主题>>>>>>>>>>>>>>>'%(page,len(menu))
    for i in menu:
        dir_name = os.path.join('meizitu',i[0])
        if not os.path.exists(dir_name):
            os.mkdir(dir_name)
        pic_nums = int(get_link(i[1]))
        print u'\n\n\n***********主题%s 一共有%d张图片***************\n'%(i[0],pic_nums)
